...
public partial class Home_BulkPDFDownloads : System.Web.UI.Page
{
    ...
    protected void CheckIsomeList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else if (exp_type == "PIPSPL" || exp_type == "PIPSPLAB")
        {
        ...
        else if (exp_type == "PID")
        {
            linkSample.NavigateUrl = "";
            sample_table_name = "";
            sample_export_type = exp_type;
        }

        else if (exp_type == "MTC")
        {
            linkSample.NavigateUrl = "";
            sample_table_name = "BULK_MTC_DOWNLOAD";
            sample_export_type = exp_type;
        }
    }
    protected void CheckSpoolList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (exp_type == "PIPSPL" || exp_type == "PIPSPLAB" || exp_type == "PIPSPLHS" || exp_type == "FLUSHING" || exp_type == "GALVANIZING" || exp_type == "PAINTING" || exp_type == "PICKLING")
        {
            linkSample.NavigateUrl = "~/Export_Format/Bulk_Spool.xlsx";
            sample_table_name = "BULK_SPOOL_DOWNLOAD";
            sample_export_type = exp_type;
    ...
    protected void CheckJointList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (exp_type == "FITUP" || exp_type == "WELDING" || exp_type == "DIM CHECK" || exp_type == "RT" || exp_type == "PT" || exp_type == "MT" || exp_type == "UT" || exp_type == "PWHT" || exp_type == "PMI" || exp_type == "HT" || exp_type == "FT" || exp_type == "PAUT")
        {
            linkSample.NavigateUrl = "~/Export_Format/Bulk_Joints_Report.xlsx";
            sample_table_name = "BULK_JOINTS_DOWNLOAD";
            sample_export_type = exp_type;
        }
    }
    ...
    protected void btnDownload_Click(object sender, EventArgs e)
    {
        try
        {
            //   CheckIsomeList.SelectedItem.ToString();
            //  CheckSpoolList.SelectedItem.ToString();
            //   CheckJointList.SelectedItem.ToString();
            ...
            else if (exp_type == "PIPSPL" || exp_type == "PIPSPLAB" || exp_type == "FLUSHING" || exp_type == "GALVANIZING" || exp_type == "PAINTING" || exp_type == "PICKLING")
            {
                tablename = "BULK_SPOOL_DOWNLOAD";
                export = "PIPSPL";
            }
            else if (exp_type == "PIPSPLHS")
            {
                tablename = "BULK_SPOOL_DOWNLOAD";
            ...
            else if (exp_type == "FITUP" || exp_type == "WELDING" || exp_type == "DIM CHECK" || exp_type == "RT" || exp_type == "PT" || exp_type == "MT" || exp_type == "UT" || exp_type == "PWHT" || exp_type == "PMI" || exp_type == "HT" || exp_type == "FT" || exp_type == "PAUT")
            {
                tablename = "BULK_JOINTS_DOWNLOAD";
                export = "JOINTS";
            }
            else if (exp_type == "MTC")
            {
                //linkSample.NavigateUrl = "~/Export_Format/Bulk_Isometric.xlsx";
                tablename = "BULK_MTC_DOWNLOAD";
                export = exp_type;
            }
            else if (exp_type == "SPL_BARCODE")
            {
    ...
    public void createTable_QR(string FileName, int spl_id, string id_1)
    {
       ...
       // var r = new Rectangle(216, 144)
        ...
        OracleConnection conn = new OracleConnection(connectionString);
        // string query = "SELECT iso_title1,spl_rev,spool,round(spl_size,2) as spl_size ,area_l1,round(weight,2),mat_type,line_no,unit FROM view_total_spool_list WHERE spl_id=" + spl_id;
        string query = "SELECT iso_title1,spl_rev,spool,spl_size,area_l1,weight,area_l2,mat_type,line_no,area_l1 FROM view_bc_image_dt WHERE spl_id=" + spl_id;
        OracleCommand cmd = new OracleCommand(query, conn);
        conn.Open();
        OracleDataReader rdr = cmd.ExecuteReader();
        // Font bigFont = new Font(Font.FontFamily.TIMES_ROMAN, 6.5f, Font.NORMAL);
        ...
        PdfPCell cell = new PdfPCell(new Paragraph("HABSHAN5/5324", headerFont));
        cell.Colspan = 2;
        ...
        var splitter = "Line:" + rdr[8].ToString() + "\n" + "\n" + "Iso:" + rdr[0].ToString() + "\n" + "\n" + "Rev:" + rdr[1].ToString() + "\n" + "\n" + "Spool:" + rdr[2].ToString() + "\n" + "\n" + "Material:" + rdr[7].ToString() + "\n" + "\n" + "Area_L2:" + rdr[6].ToString(); ;
    ...
    public void createTable(string FileName, int spl_id, string id_1)
    {
        ...
        // string query = "SELECT iso_title1,spl_rev,spool,round(spl_size,2) as spl_size ,area_l1,round(weight,2),mat_type,line_no,unit FROM view_total_spool_list WHERE spl_id=" + spl_id;
        string query = "SELECT iso_title1,spl_rev,spool,nvl(spl_size, 0) as spl_size,nvl(area_l1, 'XXX') as area_l1,nvl(weight, 0) as weight,nvl(area_l2, 'XXX') as area_l2, nvl(mat_type, 'UNK') as mat_type, nvl(line_no, '-') AS LINE_NO, nvl(AIM_NO,'-') AS UNIQUE_NO FROM view_bc_image_dt WHERE spl_id=" + spl_id;
        OracleCommand cmd = new OracleCommand(query, conn);
        ...
        PdfPCell cell = new PdfPCell(new Paragraph("HABSHAN5/5324", headfont));
        cell.Colspan = 2;
        cell.Rowspan = 4;
        cell.HorizontalAlignment = 1; //0=Left, 1=Centre, 2=Right
        cell.Border = 0;
       ...
       // bc.AltText = iso + spl;
        ...
        table.AddCell(new Paragraph("Area_l2:" + rdr[6].ToString(), bigFont));
