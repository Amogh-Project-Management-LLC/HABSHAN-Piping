...
public class FileSystemHandler : IHttpHandler
{
    ...
    string pathToConfigFile = "~/MappingFile.mapping";
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {// Handle .pdf files ;
            ...
            this.WriteFile(physicalPathToFile, "application/octet-stream", Context.Response);
        }
        ...
        {// Handle .jpg files ;
           ...
           if (Path.GetExtension(physicalPathToFile).Equals(".csv", StringComparison.CurrentCultureIgnoreCase))
        {// Handle .jpg files ;
            WriteFile(physicalPathToFile, "txt/csv", Context.Response);
        }

              if (Path.GetExtension(physicalPathToFile).Equals(".xlsx", StringComparison.CurrentCultureIgnoreCase))
        {// Handle .jpg files ;
            WriteFile(physicalPathToFile, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", Context.Response);
        }
                   if (Path.GetExtension(physicalPathToFile).Equals(".xls", StringComparison.CurrentCultureIgnoreCase))
        {// Handle .jpg files ;
            WriteFile(physicalPathToFile, "application/vnd.ms-excel", Context.Response);
        }

        if (Path.GetExtension(physicalPathToFile).Equals(".bad", StringComparison.CurrentCultureIgnoreCase))
        {// Handle .jpg files ;
            WriteFile(physicalPathToFile, "txt/csv", Context.Response);
        }
