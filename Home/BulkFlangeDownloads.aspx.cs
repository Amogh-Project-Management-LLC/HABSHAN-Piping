...
public partial class Home_BulkFlangeDownloads_1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            Master.HeadingMessage = "Bulk Flange Downloads";
    ...
    {
        if (CheckFlangeList.SelectedIndex != -1)
            return CheckFlangeList.SelectedValue;
    ...
    protected void btnDownload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if ( CheckFlangeList.SelectedItem == null )
            {
            ...
            if (exp_type == "FLANGE_BARCODE")
            {
                tablename = "BULK_FLANGE_BARCODE_DOWNLOAD";
                export = exp_type;
            }
            else if (exp_type == "FLANGE_QRCODE")
            {
                tablename = "BULK_FLANGE_BARCODE_DOWNLOAD";
                export = exp_type;
            }
            else if (exp_type == "PIDHC" || exp_type == "PIDMARKUP")
            {
                tablename = "BULK_PID_DOWNLOAD";
                export = exp_type;
            }
                ...
                try
                {
                    ...
                    if (dt.Rows.Count >= 1000)
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            {
                                createTable("BarcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), "000" + dr["ITEM_ID"].ToString());
                            }

                            if (dr["ITEM_ID"].ToString().Length == 2)
                            {
                                createTable("BarcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), "00" + dr["ITEM_ID"].ToString());
                            }

                            if (dr["ITEM_ID"].ToString().Length == 3)
                            {
                                createTable("BarcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), "0" + dr["ITEM_ID"].ToString());
                            }

                            if (dr["ITEM_ID"].ToString().Length == 4)
                            {
                                createTable("BarcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), dr["ITEM_ID"].ToString());
                            }
                        }
                    }
            ...
            if (exp_type == "FLANGE_QRCODE")
            {
                ...
                if (dt.Rows.Count >= 1000)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        if (dr["ITEM_ID"].ToString().Length == 1)
                        {
                            createTable_QR("QRcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), "000" + dr["ITEM_ID"].ToString());
                        }

                        if (dr["ITEM_ID"].ToString().Length == 2)
                        {
                            createTable_QR("QRcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), "00" + dr["ITEM_ID"].ToString());
                        }

                        if (dr["ITEM_ID"].ToString().Length == 3)
                        {
                            createTable_QR("QRcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), "0" + dr["ITEM_ID"].ToString());
                        }

                        if (dr["ITEM_ID"].ToString().Length == 4)
                        {
                            createTable_QR("QRcodeFiles", int.Parse(dr["JOINT_ID"].ToString()), dr["ITEM_ID"].ToString());
                        }
                    }
                }
    ...
    protected void CheckFlangeList_SelectedIndexChanged(object sender, EventArgs e)
    {
        string exp_type = CheckFlangeList.SelectedValue;
       
        if (exp_type == "FLANGE_BARCODE")
        {
            linkSample.NavigateUrl = "~/Export_Format/Bulk_Barcode_Flange.xlsx";
            sample_table_name = "BULK_FLANGE_BARCODE_DOWNLOAD";
        ...
        {
            linkSample.NavigateUrl = "~/Export_Format/Bulk_PID.xlsx";
            sample_table_name = "BULK_PID_DOWNLOAD";
            sample_export_type = exp_type;
        }
